...
class Player():
    ...
    def __init__(self, level, maze, tilesize):
        ...
        self.image = pygame.image.load("assets/test/chienbinh.png").convert_alpha()
    ...
    def import_player_assets(self):
        ...
        self.animations = {'idle': []}  # Simplified key for demonstration

        # Load only one type of animation for simplicity
        full_path = character_path + 'idle'  # Assume 'idle' contains your animation frames
        self.animations['idle'] = import_folder(full_path, self.tilesize)
    ...
    def input(self):
        ...
        if len(self.hintPath) > 0:
            ...
            if keys[pygame.K_UP] and not grid.walls['top']:
                self.direction = (0, -1)
                self.status = 'up'
            elif keys[pygame.K_DOWN] and not grid.walls['bottom']:
                self.direction = (0, 1)
                self.status = 'down'
            elif keys[pygame.K_RIGHT] and not grid.walls['right']:
                self.direction = (1, 0)
                self.status = 'right'
            elif keys[pygame.K_LEFT] and not grid.walls['left']:
                self.direction = (-1, 0)
                self.status = 'left'
            else:
                self.direction = (0, 0)
    ...
    def animate(self, pause):
        ...
        animation = self.animations['idle']  # Simplified for single animation key
